...
import { SiteSettingsForm } from '@/entities/site-settings-form';
import { CollapseSiteSettings } from '@/shared/ui/collapse-site-settings';
      ...
      <div className={clsx(s.container, 'desktop-only')}>
        ...
        {tab === 'variant' && (
          <div className={s.variants}>
            {siteVariants.map((variantItem) => (
              <div
                className={clsx(s.variantItem, {
                  [s.active]: variantItem.value === currentVariant,
                })}
                key={variantItem.id}
        ...
        {tab === 'fonts' && (
          <div className={s.fonts}>
            <SiteSettingsForm variant={currentVariant} />
          </div>
        )}
      </div>

      <div className={clsx(s.container, 'mobile-only')}>
        <div className={s.sidebar}>
          <Image
            src={'/webspace.svg'}
            alt="webspace logo"
        ...
        <CollapseSiteSettings title="Стилистики" icon={<StylesSettingsIcon />}>
          <div className={s.style}>
            <div className={s.styleItem}>
              <Image src={'/styles-1.png'} alt="Стилистика 1" width={392} height={192} />
              <span className={'h6'}>Стилистика 1</span>
        ...
        </CollapseSiteSettings>

        <CollapseSiteSettings title="Тематики" icon={<VariantsSettingsIcon />}>
          <div className={s.variants}>
            {siteVariants.map((variantItem) => (
              <div
                className={clsx(s.variantItem, {
        ...
        <CollapseSiteSettings title="Шрифты и цвета" icon={<SiteSettingsIcon />}>
          <div className={s.fonts}>
            <SiteSettingsForm variant={currentVariant} />
          </div>
        </CollapseSiteSettings>
